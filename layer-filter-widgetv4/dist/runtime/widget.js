System.register(["jimu-core/react","jimu-core","jimu-arcgis","jimu-ui","esri/rest/support/Query"], function(__WEBPACK_DYNAMIC_EXPORT__, __system_context__) {
...
"use strict";
/*!******************************************************************************!*\
  !*** ./your-extensions/widgets/layer-filter-widgetv3/src/runtime/widget.tsx ***!
  \******************************************************************************/
...
class Widget extends jimu_core__WEBPACK_IMPORTED_MODULE_1__.React.PureComponent {
    constructor(props) {
        ...
        this.onActiveViewChange = (jmv) => {
            if (!jmv)
                return;
            this.setState({ jimuMapView: jmv }, () => this.loadLayer());
        };
        // Robust layer resolution for Enterprise: wait for the JimuLayerView by dataSourceId.
        this.loadLayer = () => __awaiter(this, void 0, void 0, function* () {
            var _a, _b, _c, _d, _e;
            const { config } = this.props;
            const { jimuMapView } = this.state;
            if (!(config === null || config === void 0 ? void 0 : config.layerId) || !jimuMapView)
                return;
            try {
                // Prefer a waiter if available (ExB 1.10+)
                let jlv = typeof jimuMapView.whenJimuLayerViewAdded === 'function'
                    ? yield jimuMapView.whenJimuLayerViewAdded(config.layerId)
                    : null;
                // Fallbacks
                if (!jlv && typeof jimuMapView.getJimuLayerViewByDataSourceId === 'function') {
                    jlv = jimuMapView.getJimuLayerViewByDataSourceId(config.layerId);
                }
                if (!jlv) {
                    // Last resort: scan existing jimuLayerViews
                    const jlvMap = jimuMapView.jimuLayerViews || {};
                    for (const key in jlvMap) {
                        const candidate = jlvMap[key];
                        if ((candidate === null || candidate === void 0 ? void 0 : candidate.layerDataSourceId) === config.layerId) {
                            jlv = candidate;
                            break;
                        }
                    }
                ...
                if (jlv) {
                    const ds = yield jlv.createLayerDataSource();
                    const layer = ds === null || ds === void 0 ? void 0 : ds.layer;
                    if (layer) {
                        this.setState({ layer, jimuLayerView: jlv });
                        return;
                    }
                }
                // Fallback: try to match map layer by URL (dataSourceId != Layer.id)
                const ds = (_c = (_b = (_a = window.jimu) === null || _a === void 0 ? void 0 : _a.dsManager) === null || _b === void 0 ? void 0 : _b.getDataSource) === null || _c === void 0 ? void 0 : _c.call(_b, config.layerId);
                const url = (_e = (_d = ds === null || ds === void 0 ? void 0 : ds.getDataSourceJson) === null || _d === void 0 ? void 0 : _d.call(ds)) === null || _e === void 0 ? void 0 : _e.url;
                if (url) {
                    const found = this.state.jimuMapView.view.map.layers.find((lyr) => lyr.url === url);
                    if (found) {
                        this.setState({ layer: found, jimuLayerView: null });
                    }
                }
            }
            catch (error) {
        ...
        this.onFieldChange = (evt) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const q = (layer.createQuery ? layer.createQuery() : new esri_rest_support_Query__WEBPACK_IMPORTED_MODULE_4__["default"]());
                q.returnDistinctValues = true;
                q.outFields = [fieldName];
                q.where = `${fieldName} IS NOT NULL`;
                q.returnGeometry = false;
                q.orderByFields = [fieldName];
                const result = yield layer.queryFeatures(q);
        ...
        this.onSearchChange = (evt) => this.setState({ searchValue: evt.target.value });
        this.onValueSelect = (evt) => this.setState({ selectedValue: evt.target.value });
        this.applyFilter = () => __awaiter(this, void 0, void 0, function* () {
            const { selectedField, selectedValue, layer, jimuLayerView } = this.state;
            if (!layer || !selectedField || !selectedValue)
                return;
            // Escape single quotes in values to avoid expression errors
            const safeValue = String(selectedValue).replace(/'/g, "''");
            const expression = `${selectedField} = '${safeValue}'`;
            try {
                // Most compatible first
                layer.definitionExpression = expression;
                // If the jimuLayerView supports it, set there as well
                if (jimuLayerView === null || jimuLayerView === void 0 ? void 0 : jimuLayerView.setDefinitionExpression) {
                    yield jimuLayerView.setDefinitionExpression(expression);
                }
                // Zoom to extent
                const { extent } = yield layer.queryExtent({ where: expression });
                if (extent) {
                    yield this.state.jimuMapView.view.goTo(extent.expand(1.2));
                }
            }
            catch (error) {
        ...
        this.clearFilter = () => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (jimuLayerView === null || jimuLayerView === void 0 ? void 0 : jimuLayerView.setDefinitionExpression) {
                    yield jimuLayerView.setDefinitionExpression('');
                }
            }
            catch (error) {
        ...
        this.getFieldOptions = () => {
            ...
            if (!layer || !layer.fields)
                return [];
            const whitelist = (config === null || config === void 0 ? void 0 : config.allowedFields) || [];
            const compat = (f) => f.type === 'string' || f.type === 'small-integer' || f.type === 'integer' || f.type === 'double';
            const source = whitelist.length
                ? layer.fields.filter(f => whitelist.includes(f.name) && compat(f))
                : layer.fields.filter(compat);
            return source.map(f => ({ label: f.alias || f.name, value: f.name }));
        };
        this.state = {
            jimuMapView: null,
            selectedField: '',
    ...
    componentDidUpdate(prevProps) {
        var _a, _b;
        if (((_a = this.props.config) === null || _a === void 0 ? void 0 : _a.layerId) !== ((_b = prevProps.config) === null || _b === void 0 ? void 0 : _b.layerId) && this.state.jimuMapView) {
            this.loadLayer();
        }
    }
    render() {
        ...
        if (!(config === null || config === void 0 ? void 0 : config.layerId)) {
            return ((0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)("div", { css: { padding: '20px', textAlign: 'center', color: '#666' }, children: "Please configure the widget by selecting a layer." }));
        }
        const fieldOptions = this.getFieldOptions();
        const filteredValues = this.getFilteredValues();
        return ((0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)("div", { css: {
                width: '100%', height: '100%', padding: '16px', backgroundColor: '#fff',
                display: 'flex', flexDirection: 'column', gap: '12px'
            }, children: [(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(jimu_arcgis__WEBPACK_IMPORTED_MODULE_2__.JimuMapViewComponent, { useMapWidgetId: useMapWidgetIds === null || useMapWidgetIds === void 0 ? void 0 : useMapWidgetIds[0], onActiveViewChange: this.onActiveViewChange }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)("div", { children: [(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)("label", { css: { display: 'block', marginBottom: '4px', fontWeight: 'bold' }, children: "Select Field:" }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(jimu_ui__WEBPACK_IMPORTED_MODULE_3__.Select, { value: selectedField, onChange: this.onFieldChange, placeholder: "Choose a field...", style: { width: '100%' }, children: fieldOptions.map(opt => ((0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(jimu_ui__WEBPACK_IMPORTED_MODULE_3__.Option, { value: opt.value, children: opt.label }, opt.value))) })] }), selectedField && ((0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)("div", { children: [(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)("label", { css: { display: 'block', marginBottom: '4px', fontWeight: 'bold' }, children: "Search Values:" }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(jimu_ui__WEBPACK_IMPORTED_MODULE_3__.TextInput, { value: searchValue, onChange: this.onSearchChange, placeholder: "Type to search...", style: { width: '100%' } })] })), selectedField && !loading && ((0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)("div", { children: [(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)("label", { css: { display: 'block', marginBottom: '4px', fontWeight: 'bold' }, children: "Select Value:" }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(jimu_ui__WEBPACK_IMPORTED_MODULE_3__.Select, { value: selectedValue, onChange: this.onValueSelect, placeholder: "Choose a value...", style: { width: '100%' }, maxHeight: 200, children: filteredValues.map((v, i) => ((0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(jimu_ui__WEBPACK_IMPORTED_MODULE_3__.Option, { value: v, children: v }, `${v}-${i}`))) })] })), loading && ((0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)("div", { css: { textAlign: 'center', padding: '20px' }, children: (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(jimu_ui__WEBPACK_IMPORTED_MODULE_3__.Loading, {}) })), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)("div", { css: { display: 'flex', gap: '8px', marginTop: 'auto' }, children: [(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(jimu_ui__WEBPACK_IMPORTED_MODULE_3__.Button, { type: "primary", onClick: this.applyFilter, disabled: !selectedField || !selectedValue, style: { flex: 1 }, children: "Apply Filter" }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(jimu_ui__WEBPACK_IMPORTED_MODULE_3__.Button, { onClick: this.clearFilter, disabled: !selectedField, style: { flex: 1 }, children: "Clear Filter" })] })] }));
    }
}
